...
module Gen =
    ...
    let sequenceToSeq generators = collectToSeq id generators
    ...
    let collectToArr f ([<ParamArray>]xs:array<_>) =
        ...
        Gen <| fun size rnd ->
            let result = Array.zeroCreate xs.Length
            let mutable r' = rnd
    ...
    //[category: Creating generators from generators]
    [<CompiledName("Sequence")>]
    let sequenceToArr ([<ParamArray>]generators:array<Gen<_>>) = 
        collectToArr id generators
...
type Gen<'a> with
    ...
    /// Puts a in a Gen.
    static member Return (a) = Gen.constant a
    
    /// Lifted function application = apply f to a, all in the Gen applicative functor.
    static member (<*>) (f, a) = apply f a

    /// Like <*>, but puts f in a Gen first. An alias for Map but with arguments swapped.
    static member (<!>) (f, a) = Gen.constant f <*> a
    
    /// Like <*> with arguments swapped, but puts f in a Gen first.
