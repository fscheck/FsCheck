...
module Arb =
    ...
    type Default with
        ...
        ///Generates an arbitrary bool. For shrinking, false is smaller than true.
        static member Bool = Gen.elements [false; true]
        ...
        static member UInt16 = Default.Int32 |> Gen.map (abs >> uint16)
        ...
        static member UInt64 = Default.Int64 |> Gen.map (abs >> uint64)
        ...
        static member DoNotSizeUInt64 = 
            ...
            // order preserving: Int64.MinValue maps to UInt64.MinValue, Int64.MaxValue maps to UInt64.MaxValue.
            |> Gen.map (fun v -> if v < 0L then v + Int64.MaxValue + 1L |> uint64
