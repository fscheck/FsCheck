...
module Arb =
    ...
    type Default with
        ...
        static member Unit() = Arb.fromGen <| Gen.constant ()

        /// Generates bool values.
        static member Bool() = Arb.fromGen <| Gen.elements [true; false]

        /// Generates byte values that are unrestricted by size.
        static member Byte() =
            Arb.fromGenShrink ( Gen.choose (int Byte.MinValue, int Byte.MaxValue) |> Gen.map byte,
                                Shrink.unsignedNumber)

        /// Generates sbyte values that are unrestricted by size.
        static member SByte() =
            Arb.fromGenShrink ( Gen.choose (int SByte.MinValue, int SByte.MaxValue) |> Gen.map sbyte,
                                Shrink.signedNumber)

        ///Generates int16 values that are between -size and size.
        static member Int16() =
             Arb.fromGenShrink( Gen.sized( fun s -> Gen.choose(0, s) |> Gen.map int16),
                                Shrink.signedNumber )

        ///Generate arbitrary int16 that is uniformly distributed in the whole range of int16 values.
        static member DoNotSizeInt16() =
            let gen = Gen.choose(int Int16.MinValue, int Int16.MaxValue) |> Gen.map int16
            Arb.fromGenShrink(gen, Shrink.signedNumber)
        ...
        static member DoNotSizeUInt16() =
            let gen = Gen.choose(0, int UInt16.MaxValue) |> Gen.map uint16
            Arb.fromGenShrink(gen, Shrink.unsignedNumber)
            |> Arb.convert DoNotSize DoNotSize.Unwrap
         
        ///Generates int32 values that are between -size and size.
        static member Int32() = 
            Arb.fromGenShrink (Gen.sized (fun n -> Gen.choose (-n,n)),
                               Shrink.signedNumber)

        ///Generate arbitrary int32 that is unrestricted by size.
        static member DoNotSizeInt32() =
            let gen = Gen.choose(Int32.MinValue, Int32.MaxValue)
            Arb.fromGenShrink(gen, Shrink.signedNumber)
            |> Arb.convert DoNotSize DoNotSize.Unwrap

        ///Generates uint32 values that are between 0 and size.
        static member UInt32() =
            let gen = Gen.sized( fun s -> Gen.choose(0, s) |> Gen.map uint32)
            Arb.fromGenShrink(gen, Shrink.unsignedNumber)

        ///Generate arbitrary uint32 that is unrestricted by size.
        static member DoNotSizeUInt32() =
            let gen = Gen.choose64(0L, int64 UInt32.MaxValue) |> Gen.map uint32
            Arb.fromGenShrink(gen, Shrink.unsignedNumber)
            |> Arb.convert DoNotSize DoNotSize.Unwrap
        ...
        static member Int64() =
            ...
            let gen = Gen.sized( fun s -> Gen.choose64(int64 -s, int64 s))
            Arb.fromGenShrink(gen, Shrink.signedNumber)

        ///Generate arbitrary int64 that is unrestricted by size.
        static member DoNotSizeInt64() =
            let gen = Gen.choose64(Int64.MinValue, Int64.MaxValue) 
            Arb.fromGenShrink (gen,Shrink.signedNumber)
            |> Arb.convert DoNotSize DoNotSize.Unwrap
        ...
        static member DoNotSizeDecimal() =
            ...
            Arb.fromGenShrink (generator, Default.Decimal().Shrinker)
            |> Arb.convert DoNotSize DoNotSize.Unwrap
        ...
        static member Array2D() = 
            ...
            Arb.fromGenShrink (Gen.array2DOf generate,shrinkArray2D)
        ...
        static member DateTimeOffset() =
            ...
            Arb.fromGenShrink (genDate, shrink)
        ...
        static member List() =
            Default.FsList() 
            |> Arb.convert Enumerable.ToList Seq.toList
        ...
        static member Derive() =
            ...
            //is used properly.
            let generator = ReflectiveGenerator.reflectGenObj getGenerator
            { new Arbitrary<'a>() with
                override __.Generator = generator typeof<'a> |> Gen.map unbox<'a>
                override __.Shrinker a = ReflectiveShrinker.reflectShrink getShrink a
            }
