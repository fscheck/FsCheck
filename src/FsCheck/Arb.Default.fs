...
module Arb =
    ...
    type Default with
         ...
         static member Unit() = 
             Arb.fromGen <| Gen.constant ()

         ///Generates bool values.
         static member Bool() = 
             Arb.fromGen <| Gen.elements [true; false]

         ///Generates byte values that are unrestricted by size.
         static member Byte() =
             Arb.fromGenShrink ( Gen.choose (0,255) |> Gen.map byte, //this is now size independent - 255 is not enough to not cover them all anyway 
                                int >> shrink >> Seq.map byte)

         ///Generates sbyte values that are unrestricted by size.
         static member SByte() =
             Arb.fromGenShrink ( Gen.choose (-128,127) |> Gen.map sbyte,
                                 int >> shrink 
         ...
         static member DoNotSizeInt16() =
             ...
             |> Arb.convert (int16 >> DoNotSize) (DoNotSize.Unwrap >> int)
         ...
         static member DoNotSizeUInt16() =
             ...
             Arb.fromGenShrink(gen, shrink)
             |> Arb.convert (uint16 >> DoNotSize) (DoNotSize.Unwrap >> int)
         ...
         static member DoNotSizeUInt32() =
             ...
             Arb.fromGenShrink(gen, shrink)
             |> Arb.convert (uint32 >> DoNotSize) DoNotSize.Unwrap
         ...
         static member DoNotSizeInt64() =
             ...
             Arb.fromGenShrink (gen,Shrink.number)
             |> Arb.convert DoNotSize DoNotSize.Unwrap
