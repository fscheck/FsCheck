...
- The sequence of shrinks generated by the built-in shrinkers may be long, and evaluating the whole sequence may slow down tests. However these sequences should never be infinite.
- Your custom shrinker function may be called for each test, even if the tests are not failing. FsCheck may store the returned sequence, but it will only be evaluated once a test fails.

For example: for 200 tests that would normally take 1 second and pass, the following `Arbitrary` instance will make those tests take 10 seconds if the `Seq.length` line is uncommented.
*)
...
(**
Another common mistake when writing custom shrinkers is to return a shrink which has already been tried, in which case shrinking may go on infinitely. A custom shrinker should ensure the values returned are never larger than or equal to its input.
