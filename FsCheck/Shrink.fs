...
let shrink (test : 'a -> bool) (x : 'a) : 'a =
    //let f = unbox >> test
    if (debug) then printfn "shrinking value %A of type %A" x (x.GetType())
...
//
//
//let rec private children0 (tFind : Type) (t : Type) : (obj -> list<obj>) =
//        if tFind = t then
//            fun o -> [o]
//        else
//            children1 tFind t
//
//and private children1 (tFind : Type) (t : Type) =
//    if t.IsArray then
//        let f = children0 tFind (t.GetElementType())
//        fun o ->
//            let x = o :?> Array
...
//        fun _ -> []
//
//let private hunt (t : Type) : ShrinkType =
//    let cs = children1 t t
//    fun test o ->
//        let rec f xs =
//            match xs with
...
//
//
//let rec private getShrink (t : Type) : Lazy<ShrinkType> =
//        let ts = t.ToString()
//        match (!shrinkMap).TryFind ts with
//        | Some v -> v
//        | None ->
...
//            res
//
//and private arrayShrink (g : int -> Lazy<ShrinkType>) =
//    fun test (o : obj) -> 
//        let cs = o :?> Array
//        let mutable changed = false
//        for i in 0 .. cs.Length - 1 do
...
//        if changed then Some (box cs) else None
//
//and private listShrink (destroy : obj -> obj[]) (create : obj[] -> obj) (fields : list<Type>) =
//    let ss = [| for i in fields -> getShrink i |]
//    fun test o ->
//        let cs = destroy o
//        let mutable changed = false
...
//// Try to shrink by first looking for children
//// Then if that doesn't work, try something more specific
//and private genShrink (t : Type) : ShrinkType =
//    let gen = hunt t
//    let spec = newShrink t
//    let f g x = match g x with None -> false,x | Some y -> true,y
//    let rec fs g x = match g x with None -> false,x | Some y -> true,snd (fs g y)
...
//        if b1 || b2 then Some o else None
//
//and private newShrink (t : Type) : ShrinkType =
//    if t.IsArray then
//        let t2 = t.GetElementType()
//        let inner = getShrink t2
//        fun test o -> //None
...
//            if x.Length = 0 then None
//            else
//// do not try to shrink to empty array: since internally FsCheck uses obj arrays to invoke property functions
////reflectively, such an array cannot be made empty. Otherwise a missingmethod exception will be thrown.
////                let empty = box <| Array.CreateInstance(t2, 0)
////                if test empty then Some empty
////                else
//                    let x2 = Array.CreateInstance(t2, x.Length)
...
//// you want to find the smallest value value such that
//// test returns true, assuming (test x) = true
//let internal shrink (test : 'a -> bool) (x : 'a) : 'a =
//    //let f = unbox >> test
//    printfn "shrink called with arg %A" x
//    let f = fun x -> let v = test (unbox x) 
//                     if debugNeilCheck then printf "%s" (if v then "#" else ".")
//                     v
//    unbox <| shrinkObj (typeof<'a>) (f) (box x)
