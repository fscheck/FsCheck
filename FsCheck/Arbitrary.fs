...
type Default =
    ...
    static member Float() = 
        { new Arbitrary<float>() with
            override x.Generator = 
                frequency   [(6, map3 Default.fraction generate generate generate)
    ...
    static member Option() = 
        { new Arbitrary<option<'a>>() with
            override x.Generator = frequency [(1, gen { return None }); (7, Gen.map Some generate)]
    ...
    static member FsList() = 
        { new Arbitrary<list<'a>>() with
            override x.Generator = listOf generate
    ...
    static member Object() =
        { new Arbitrary<obj>() with
            override x.Generator = 
                oneof [ Gen.map box <| generate<char> ; Gen.map box <| generate<string>; Gen.map box <| generate<bool> ]
            override x.Shrinker o =
                seq {
                    match o with
    ...
    static member Arrow() = 
        { new Arbitrary<'a->'b>() with
            override x.Generator = promote (fun a -> variant a generate) //coarbitrary a arbitrary)
        }
    ...
    static member Interval() =
        { new Arbitrary<Interval>() with
            override  x.Generator = 
                gen { let! start,offset = two generate
