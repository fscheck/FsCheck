...
module Arbitrary =
    ...
    let arrayOfSize (g: Gen<'a>) n : Gen<'a[]> = vector g n |> map Array.ofList
    ...
    let setOf (g: Gen<'a>) : Gen<Set<'a>> = listOf g |> map Set.ofList
    ...
    /// Generate a set
    let set() = arbitrary |> map Set.ofList
    /// Shrink a set
    let shrinkSet s = s |> Set.toList |> shrink |> Seq.map Set.ofList

    /// Generate a map
    let map() = arbitrary |> map Map.ofList
    /// Shrink a map
    ...
    let enumOf<'enumType when 'enumType :> Enum>() : Gen<'enumType>  =
       Gen.map unbox (ReflectArbitrary.enumOfType (typeof<'enumType>))
    ...
    type Arbitrary() =
        ...
        static member Float() = 
            { new Arbitrary<float>() with
                override x.Arbitrary = 
                    frequency   [(6, map3 fraction arbitrary arbitrary arbitrary)
        ...
        static member Char() = 
            { new Arbitrary<char>() with
                override x.Arbitrary = Gen.map char (choose (int Char.MinValue, 127))
        ...
        static member Option() = 
            { new Arbitrary<option<'a>>() with
                override x.Arbitrary = frequency [(1, gen { return None }); (3, Gen.map Some arbitrary)]
        ...
        static member Array() =
            { new Arbitrary<'a[]>() with
                override x.Arbitrary = arbitrary |> Gen.map List.toArray
        ...
        static member Function() =
            { new Arbitrary<Function<'a,'b>>() with
                override x.Arbitrary = Gen.map toFunction arbitrary
                override x.Shrink f = 
