...
module Gen =
    ...
    let Elements (l:list<char>) =
        ...
        lazy (  assertTrue ( Gen.elements l
                        |> sample 50
                        |> Seq.forall (fun v -> List.contains v l) ) )
    ...
    [<Fact>]
    let ``bind shrinks``() =
        let gen = Gen.choose64 (0L,5L) >>= (fun i -> Gen.choose64 (-i,i) |> Gen.map (fun j -> (i,j)))
        assertShrinksFrom 20 gen (fun v shrunkv -> shrunkv <! v)
    ...
    let Frequency (frequencies:list<PositiveInt*string>) =
        let generatedValues = frequencies |> List.map snd
        not generatedValues.IsEmpty ==>
         lazy ( assertTrue ( frequencies
                        |> List.map (fun (freq,s) -> (freq.Get,Gen.constant s))
                        |> Gen.frequency
                        |> sample 100
                        |> Seq.forall (fun v -> List.contains v generatedValues) ))

    [<Property>]
    let ``Frequency shrinks`` (NonEmptyArray frequencies:NonEmptyArray<PositiveInt>) =
        let gen = frequencies 
                  |> Array.mapi (fun i (PositiveInt f) -> f, Gen.constant i) 
                  |> Gen.frequency 

        // the problem here is that frequence is choose (1,tot) >>= pick 
        // so when shrinking with higher weights, the bind will retry the same generator
        // multiple times when the value generated by choose is shrunk 
        assertShrinksFrom 1 gen (fun v shrunkv -> shrunkv <! v)

    [<Fact>]
    let ``Frequency shrinks repro`` () =
        let gen = [| 2;4 |] 
                  |> Array.mapi (fun i f -> f, Gen.constant i) 
                  |> Gen.frequency 
        assertShrinksFrom 1 gen (fun v shrunkv -> shrunkv <! v)
    ...
    // these types were taken from the FSharpLu project, as there was some regression
